...
| `pyversion.sh` | Bash | Prints Python version string; supports `--json` output |
...
**Default output** (one line to stdout):
...
| Flag | Description |
|------|-------------|
| `--json` | Output version and OS info as a single-line JSON object |

#### `--json` output

```bash
bash pyversion.sh --json
```
...
The JSON fields are identical to those produced by `pyversion.py --json`. OS detection uses the same fallback logic via a Python one-liner.
...
| `1` | Python not found, version query failed, unknown flag, or too many arguments |

Unknown flags (e.g. `--verbose`) print an error message to stderr and exit `1`. Passing more than one argument also exits `1`.
...
├── pyversion.sh          # Bash wrapper — prints version string (with --json support)
